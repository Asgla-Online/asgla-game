...
namespace CharacterCreator2D {
    public class CharacterViewer : MonoBehaviour {
        ...
        private void instantiateSlotsMaterial() {
            ...
            foreach (Equipment cat in Enum.GetValues(typeof(Equipment))) {
        ...
        private Equipment getMaterialsCategory(Material material) {
            foreach (Equipment s in Enum.GetValues(typeof(Equipment))) {
        ...
        public void RepaintTintColor() {
            foreach (Equipment c in Enum.GetValues(typeof(Equipment))) {
        ...
        public Part GetAssignedPart(Equipment category) {
        ...
        /// </summary>
        /// <param name="equipment">Desired SlotCategory.</param>
        ...
        public void EquipPart(Equipment equipment, Part part) {
            ...
            if (part != null && !part.supportedBody.Contains(this.bodyType)) {
                ...
                if (altpart == null)
                    Debug.LogFormat("part '{0}' on slot category '{1}' doesn't support body type {2}", part, equipment, this.bodyType);
                else
                    Debug.LogFormat("part '{0}' on slot category '{1}' doesn't support body type {2}. it will be replaced with {3}", part, equipment, this.bodyType, altpart.name);
                part = altpart;
            }
            ...
            switch (equipment) {
                case Equipment.MainHand: {
                    ...
                    Weapon offweapon = (Weapon)slots.GetSlot(Equipment.OffHand).assignedPart;
                    if (mainweapon != null && mainweapon.weaponCategory == Asgla.Data.Type.Weapon.TwoHanded)
                        equipWeapon(Equipment.OffHand, null);
                    else if (mainweapon != null && offweapon != null && offweapon.weaponCategory == Asgla.Data.Type.Weapon.Bow)
                        equipWeapon(Equipment.OffHand, null);
                    else if (mainweapon != null && mainweapon.weaponCategory == Asgla.Data.Type.Weapon.Rifle)
                        equipWeapon(Equipment.OffHand, null);
                    equipWeapon(Equipment.MainHand, mainweapon);
                    break;
                }
                case Equipment.OffHand: {
                    Weapon mainweapon = (Weapon)slots.GetSlot(Equipment.MainHand).assignedPart;
                    Weapon offweapon = (Weapon)part;
                    if (offweapon != null && offweapon.weaponCategory == Asgla.Data.Type.Weapon.Bow)
                        equipWeapon(Equipment.MainHand, null);
                    else if (offweapon != null && mainweapon != null && mainweapon.weaponCategory == Asgla.Data.Type.Weapon.TwoHanded)
                        equipWeapon(Equipment.MainHand, null);
                    else if (offweapon != null && mainweapon != null && mainweapon.weaponCategory == Asgla.Data.Type.Weapon.Rifle)
                        equipWeapon(Equipment.MainHand, null);
                    equipWeapon(Equipment.OffHand, offweapon);
                    break;
                }
        ...
        private void equipWeapon(Equipment slotCategory, Weapon weapon) {
            ...
            switch (slotCategory) {
                case Equipment.MainHand:
                    if (weapon.weaponCategory == Asgla.Data.Type.Weapon.OneHanded ||
                        weapon.weaponCategory == Asgla.Data.Type.Weapon.TwoHanded ||
                        weapon.weaponCategory == Asgla.Data.Type.Weapon.Gun ||
                        weapon.weaponCategory == Asgla.Data.Type.Weapon.Rifle)
                        links = SetupData.rWeaponLink;
                    break;
                case Equipment.OffHand:
                    if (weapon.weaponCategory == Asgla.Data.Type.Weapon.Bow)
                        links = SetupData.bowLink;
                    else if (weapon.weaponCategory == Asgla.Data.Type.Weapon.Shield)
                        links = SetupData.shieldLink;
                    else if (weapon.weaponCategory == Asgla.Data.Type.Weapon.OneHanded ||
                             weapon.weaponCategory == Asgla.Data.Type.Weapon.Gun)
            ...
            foreach (Sprite s in weapon.sprites) {
                if (!links.ContainsKey(s.name)) {
                    Debug.LogError($"Weapon sprite Part not found {s.name}");
                } else {
            ...
            if (slotCategory == Equipment.MainHand)
                muzzlefx = this.transform.Find(SetupData.muzzleFXLinks[Equipment.MainHand]);
            if (slotCategory == Equipment.OffHand)
                muzzlefx = this.transform.Find(SetupData.muzzleFXLinks[Equipment.OffHand]);
        ...
        private void resetWeaponRenderer(PartSlot slot) {
            ...
            /*if (slot == this.slots.GetSlot(Equipment.MainHand)) {
                foreach (string k in SetupData.rWeaponLink.Keys)
                    this.transform.Find(SetupData.rWeaponLink[k]).GetComponent<SpriteRenderer>().sprite = null;
            } else if (slot == this.slots.GetSlot(Equipment.OffHand)) {
                foreach (string k in SetupData.bowLink.Keys)
                    this.transform.Find(SetupData.bowLink[k]).GetComponent<SpriteRenderer>().sprite = null;
                foreach (string k in SetupData.shieldLink.Keys)
                    this.transform.Find(SetupData.shieldLink[k]).GetComponent<SpriteRenderer>().sprite = null;
        ...
        private void equipSkinDetails(Part part) {
            if (part != null && part.category != Category.SkinDetail)
                return;

            PartSlot slot = this.slots.GetSlot(Equipment.SkinDetail);
            if (slot == null)
                return;
        ...
        private void equipCape(Part part) {
            if (part != null && part.category != Category.Cape)
                return;

            PartSlot slot = this.slots.GetSlot(Equipment.Cape);
            if (slot == null)
                return;
        ...
        private void equipSkirt(Part part) {
            if (part != null && part.category != Category.Skirt)
                return;

            PartSlot slot = this.slots.GetSlot(Equipment.Skirt);
            if (slot == null)
                return;
        ...
        private void equipPart(Equipment slotCategory, Part part) {
            ...
            foreach (Sprite s in part.sprites) {
                if (!links.ContainsKey(s.name)) {
                    Debug.LogError($"Sprite Part not found {s.name}");
                } else {
                    this.transform.Find(links[s.name]).GetComponent<SpriteRenderer>().sprite = s;
                }
            }
        ...
        public Color GetPartColor(Equipment slotCategory, string colorCode) {
        ...
        public void SetPartColor(Equipment slotCategory, string colorCode, Color color) {
            if (slotCategory == Equipment.MainHand || slotCategory == Equipment.OffHand)
                setWeaponColor(slotCategory, colorCode, color);
            else if (slotCategory == Equipment.SkinDetail)
        ...
        public void SetPartColor(Equipment slotCategory, Color color1, Color color2, Color color3) {
        ...
        private void setWeaponColor(Equipment slotCategory, string colorCode, Color color) {
            if (colorCode == ColorCode.Color1) {
                if (slotCategory == Equipment.MainHand)
                    this.transform.Find(SetupData.weaponFXLinks[Equipment.MainHand]).GetComponent<SpriteRenderer>().color = color;
                else if (slotCategory == Equipment.OffHand)
                    this.transform.Find(SetupData.weaponFXLinks[Equipment.OffHand]).GetComponent<SpriteRenderer>().color = color;
            }
        ...
        private void setPartColor(Equipment slotCategory, string colorCode, Color color) {
            PartSlot slot = this.slots.GetSlot(slotCategory);
            if (SetupData.colorableSpriteLinks.ContainsKey(slotCategory)) {
        ...
        private void emote(EmoteIndex i, float duration, bool isAnimation) {
            ...
            if (i.eyebrowPart != null)
                EquipPart(Equipment.Eyebrow, i.eyebrowPart);
            else
                EquipPart(Equipment.Eyebrow, defaultEmote.eyebrowPart);
            if (i.eyesPart != null)
                EquipPart(Equipment.Eye, i.eyesPart);
            else
                EquipPart(Equipment.Eye, defaultEmote.eyesPart);
            if (i.nosePart != null)
                EquipPart(Equipment.Nose, i.nosePart);
            else
                EquipPart(Equipment.Nose, defaultEmote.nosePart);
            if (i.mouthPart != null)
                EquipPart(Equipment.Mouth, i.mouthPart);
            else
                EquipPart(Equipment.Mouth, defaultEmote.mouthPart);
            if (i.earPart != null)
                EquipPart(Equipment.Ear, i.earPart);
            else
                EquipPart(Equipment.Ear, defaultEmote.earPart);
            if (duration > 0)
        ...
        private void getDefaultEmote() {
            Part p;
            p = GetAssignedPart(Equipment.Eyebrow);
            defaultEmote.eyebrowPart = p == null ? null : p;
            p = GetAssignedPart(Equipment.Eye);
            defaultEmote.eyesPart = p == null ? null : p;
            p = GetAssignedPart(Equipment.Nose);
            defaultEmote.nosePart = p == null ? null : p;
            p = GetAssignedPart(Equipment.Mouth);
            defaultEmote.mouthPart = p == null ? null : p;
            p = GetAssignedPart(Equipment.Ear);
