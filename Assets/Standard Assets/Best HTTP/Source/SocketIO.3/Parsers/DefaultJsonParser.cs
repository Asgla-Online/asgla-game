	...
	public sealed class DefaultJsonParser : IParser {
		...
		public IncomingPacket MergeAttachements(SocketManager manager, IncomingPacket packet) {
			...
			for (int i = 0; i < packet.Attachements.Count; ++i) {
				...
				payload = payload.Replace(placeholder,
					"\"" + Convert.ToBase64String(data.Data, data.Offset, data.Count) + "\"");
			}
		...
		public IncomingPacket Parse(SocketManager manager,
			BufferSegment data,
		...
		public OutgoingPacket CreateOutgoing(Socket socket,
		...
		private (string, object[]) ReadData(SocketManager manager, IncomingPacket packet, string payload) {
			...
			switch (packet.SocketIOEvent) {
				...
				case SocketIOEventTypes.Ack:
					...
					args = ReadParameters(socket, subscription, JSON.LitJson.JsonMapper.ToObject<List<object>>(payload),
						0);
					...
					if (packet.AttachementCount == 0 || packet.Attachements != null) {
						try {
							...
							HTTPManager.Logger.Exception("DefaultJsonParser",
								string.Format("ReadParameters with eventName: {0}", eventName), ex);
						}
					}
