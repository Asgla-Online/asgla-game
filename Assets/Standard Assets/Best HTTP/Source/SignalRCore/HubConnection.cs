...
namespace BestHTTP.SignalRCore {
	public sealed class HubConnection : BestHTTP.Extensions.IHeartbeat {
		...
		private long InvokeImp(string target,
			object[] args,
		...
		public UpStreamItemController<TResult> GetUpStreamController<TResult>(string target,
			int paramCount,
			bool downStream,
			object[] args) {
			Future<TResult> future = new Future<TResult>();
		...
		private void SetState(ConnectionStates state, string errorReason = null, bool allowReconnect = true) {
			if (string.IsNullOrEmpty(errorReason))
				HTTPManager.Logger.Information("HubConnection",
					string.Format("SetState - from State: '{0}' to State: '{1}', allowReconnect: {2}", this.State,
						state, allowReconnect), this.Context);
			else
				HTTPManager.Logger.Information("HubConnection",
					string.Format(
						"SetState - from State: '{0}' to State: '{1}', errorReason: '{2}', allowReconnect: {3}",
						this.State, state, errorReason, allowReconnect), this.Context);
					...
					if (errorReason == null) {
						...
						if (allowReconnect && this.ReconnectPolicy != null &&
							...
							if (nextAttempt == null) {
								...
								HTTPManager.Logger.Information("HubConnecction",
		...
		public Task<TResult> InvokeAsync<TResult>(string target,
		...
		public Task<object> SendAsync(string target,
			CancellationToken cancellationToken = default,
			params object[] args) {
