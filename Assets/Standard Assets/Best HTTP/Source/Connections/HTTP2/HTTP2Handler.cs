...
namespace BestHTTP.Connections.HTTP2 {
	public sealed class HTTP2Handler : IHTTPRequestHandler {
				...
				using (WriteOnlyBufferedStream bufferedStream =
					...
					while (this.isRunning) {
						...
						if (!atLeastOneStreamHasAFrameToSend) {
							...
							var nextDueClientInteractionAt =
								sendPingAt < disconnectByIdleAt ? sendPingAt : disconnectByIdleAt;
						...
						for (int i = 0; i < this.outgoingFrames.Count; ++i) {
							...
							if (frame.PayloadLength > 0) {
								bufferedStream.Write(frame.Payload, (int) frame.PayloadOffset,
		...
		private void OnRemoteSettingChanged(HTTP2SettingsRegistry registry,
