...
namespace AsglaUIEditor.UI {
	...
	internal class UITooltipLineContentDrawer : PropertyDrawer {
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
			...
			UITooltipLines.LineStyle style =
				(UITooltipLines.LineStyle) property.FindPropertyRelative("LineStyle").enumValueIndex;

			// Using BeginProperty / EndProperty on the parent property means that
			// prefab override logic works on the entire property.
			EditorGUI.BeginProperty(position, label, property);

			EditorGUI.HelpBox(new Rect(position.x + 14f, position.y, position.width - 14f, position.height), "",
				MessageType.None);

			// Draw label
			//EditorGUI.PrefixLabel(position, GUIUtility.GetControlID(FocusType.Passive), label);
			position = new Rect(position.x, position.y + 6f + Spacing, position.width - 8f,
			...
			if (!isSpacer) {
				...
				if (style == UITooltipLines.LineStyle.Custom) {
					if (UITooltipManager.Instance != null) {
						DrawCustomStyleField(position, property.FindPropertyRelative("CustomLineStyle"));
						position = new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight + Spacing,
							position.width, EditorGUIUtility.singleLineHeight);
					} else {
						EditorGUI.PropertyField(position, property.FindPropertyRelative("CustomLineStyle"),
							new GUIContent("Style Name"));
						position = new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight + Spacing,
							position.width, EditorGUIUtility.singleLineHeight);
					}
				}
			...
			position = new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight + Spacing, position.width,
				EditorGUIUtility.singleLineHeight);
