		...
		public static float Apply(TweenEasing e, float t, float b, float c, float d) {
			switch (e) {
				...
				case TweenEasing.InOutBounce: {
					if (t < d / 2f) return Apply(TweenEasing.InBounce, t * 2f, 0f, c, d) * .5f + b;
					return Apply(TweenEasing.OutBounce, t * 2f - d, 0f, c, d) * .5f + c * .5f + b;
				}
				...
				case TweenEasing.InOutElastic: {
					...
					if (t < 1f)
						return -.5f *
							(a * Mathf.Pow(2f, 10f * (t -= 1f)) * Mathf.Sin((t * d - s) * (2f * Mathf.PI) / p)) + b;
					return a * Mathf.Pow(2f, -10f * (t -= 1f)) * Mathf.Sin((t * d - s) * (2f * Mathf.PI) / p) * .5f +
					       c + b;
				}
