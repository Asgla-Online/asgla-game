    ...
    public class UIItemSlot : UISlotBase, IUIItemSlot {
        ...
        public override bool CanSwapWith(Object target) {
            if (target is IUIItemSlot) {
                ...
                if (target is ItemData) {
                    return (target as UIEquipSlot).CheckEquipType(this.GetItemInfo());
                    //return (target as InventoryData).CheckEquipType(this.GetItemInfo());
                }
		...
		public static void PrepareTooltip(ItemData itemInfo) {
            if (itemInfo == null)
                return;
            ...
            // Item types
            UITooltip.AddLineColumn("Type", "ItemAttribute");
            UITooltip.AddLineColumn("Sub Type", "ItemAttribute");

            if (itemInfo.Type.Category == Category.Weapon) {
                UITooltip.AddLineColumn("100 Damage", "ItemAttribute");
                UITooltip.AddLineColumn("1.0 Attack speed", "ItemAttribute");
                UITooltip.AddLine("(1.0 damage per second)", "ItemAttribute");
            } else {
                UITooltip.AddLineColumn("1.0 Armor", "ItemAttribute");
                UITooltip.AddLineColumn("1.0 Block", "ItemAttribute");
            }

            UITooltip.AddSpacer();

            UITooltip.AddLine("+10 Stamina", "ItemStat");
            UITooltip.AddLine("+10 Strength", "ItemStat");
