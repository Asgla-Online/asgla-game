...
namespace AsglaUIEditor.UI {
	...
	public class ColorBlockExtendedDrawer : PropertyDrawer {
		...
		protected static bool m_HasCopy;
		...
		public override void OnGUI(Rect rect, SerializedProperty prop, GUIContent label) {
			...
			// Copy button
			if (GUI.Button(new Rect(controlRect.x, controlRect.y, controlRect.width / 2f - 2f, controlRect.height),
					"Copy", EditorStyles.miniButton))
			...
			if (GUI.Button(
					new Rect(controlRect.x + (controlRect.width / 2f + 4f), controlRect.y, controlRect.width / 2f - 2f,
						controlRect.height), "Paste", EditorStyles.miniButton))
