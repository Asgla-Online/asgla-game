...
public class ResourcePathDrawer : PropertyDrawer {
	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
		...
		if (property.propertyType == SerializedPropertyType.String) {
			...
			DragAndDropArea(position, property);
		} else {
	...
	private void DragAndDropArea(Rect position, SerializedProperty property) {
		...
		Rect drop_area = new Rect(position.x + EditorGUIUtility.labelWidth, position.y,
			EditorGUIUtility.currentViewWidth - 20f - EditorGUIUtility.labelWidth, EditorGUIUtility.singleLineHeight);
