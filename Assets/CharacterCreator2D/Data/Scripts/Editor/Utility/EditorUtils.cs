...
namespace CharacterEditor2D {
	public static class EditorUtils {
		...
		public static List<T> GetPrefabs<T>(string path, bool readThroughFolders) where T : UnityEngine.MonoBehaviour {
			...
			string[] files = readThroughFolders
				? Directory.GetFiles(path, "*.prefab", SearchOption.AllDirectories)
		...
		public static List<T> GetScriptables<T>(string path, bool readThroughFolders)
			...
			List<T> val = new List<T>();
			string[] files = readThroughFolders
				? Directory.GetFiles(path, "*.asset", SearchOption.AllDirectories)
