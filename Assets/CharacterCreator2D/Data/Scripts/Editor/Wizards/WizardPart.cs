		...
		protected void createPart() {
			try {
				...
				if (_templates[_stemp] is WeaponTemplate) {
					...
					filename = WizardUtils.PartFolder + "/" + _packstring[_spack] + "/Weapon/" +
					           getDirectoryName(twtemplate.weaponCategory) + "/" + _partname + ".asset";
				} else
					filename = WizardUtils.PartFolder + "/" + _packstring[_spack] + "/" +
					           getDirectoryName(_templates[_stemp].category) + "/" + _partname + ".asset";
				...
				if (File.Exists(filename)) {
					if (!EditorUtility.DisplayDialog("Replace Part", "the part you wish to create is already exist",
						"Replace", "Cancel"))
						return;
				}

				Part tpart = _templates[_stemp] is WeaponTemplate
					? EditorUtils.LoadScriptable<Weapon>(filename)
				...
				//..sprites
				if (_texture != null &&
				    _templates[_stemp].category != PartCategory.SkinDetails) //..skin details doesn't need to be sliced
				{
