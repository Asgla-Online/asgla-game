...
namespace CharacterEditor2D {
	...
	public static class PartRefCustomMenu {
		...
		private class InternalProcessor : IActiveBuildTargetChanged, IPreprocessBuildWithReport {
			...
			void IActiveBuildTargetChanged.
				OnActiveBuildTargetChanged(BuildTarget previousTarget, BuildTarget newTarget) {
		...
		private static void UpdatePartRefererUsageCheckbox() {
			...
			Menu.SetChecked(MENU_PARTREFERER_ENABLE,
				!string.IsNullOrEmpty(definesString) && definesString.Contains(CC2D_RES_SYMBOL));
			EditorApplication.update -= UpdatePartRefererUsageCheckbox;
		}
		...
		private static void CheckPartRefererUsage() {
			...
			if (EditorPrefs.HasKey(CC2D_RES_KEY)) {
				...
				if (m_isUsageEnabled) {
					if (EditorUtility.DisplayDialog("Refresh Part Referer Confirmation",
						"Would You like to refresh Part Referers?", "Yes", "No")) {
						RefreshPartReferers();
					}
				} else {
					if (AssetDatabase.IsValidFolder(CC2D_RES_PATH)) {
						if (EditorUtility.DisplayDialog("Cleanup Part Referer Confirmation",
							"Would You like to cleanup Part Referers?", "Yes", "No")) {
							CleanupPartReferers();
						}
					}
				}
        ...
        private static string InitPartReferer(Part partObject, Dictionary<string, PartReferer> lastPartRefererDict)
        {
            ...
            if (EditorUtils.MakeSureAssetFolderExist(partRefPath.Remove(partRefPath.LastIndexOf("/"))))
            {
                ...
                partObject.datapath =
 CC2D_RES_FOLDER + "/" + relativePartRefPath.Remove(relativePartRefPath.LastIndexOf(".asset"));
            }
            return partRefPath;
        }
        ...
        private static Dictionary<string, PartReferer> GetCurrentPartRefererDict()
        {
            ...
            {
                string[] foundPartReferers =
 AssetDatabase.FindAssets(SEARCH_PARTREFERER_FILTER, m_searchPartRefererPath);
