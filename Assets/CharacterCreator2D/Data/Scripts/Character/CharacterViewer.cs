...
namespace CharacterCreator2D {
	...
	public partial class CharacterViewer : MonoBehaviour {
		...
		[SerializeField] [FormerlySerializedAs("setupData")]
		...
		[SerializeField] private BodySegmentScaler _bodyscaler = new BodySegmentScaler();

		[SerializeField] private Color _skincolor = Color.gray;

		[SerializeField] private Color _tintcolor = Color.white;

		[SerializeField] [HideInInspector] private bool _isBaked;
		...
		private void BakedSlotsToSlots() {
			foreach (var cat in CharacterUtility.slotCategories) {
				...
				if (!string.IsNullOrEmpty(partSlot.AssignedPartName)) {
					partSlot.assignedPart =
						PartList.Static.FindPart(partSlot.AssignedPartName, partSlot.AssignedPackageName, cat);
				} else {
					partSlot.assignedPart = null;
				}
			}
