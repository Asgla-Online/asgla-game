...
namespace CharacterCreator2D.Utilities.Humanoid {
	public class HumanoidSlotRenderer {
		...
		public static Texture2D RenderToTexture2D(CharacterViewer character,
		...
		private RenderTexture RenderSlot(CharacterViewer character, SlotCategory category) {
			...
			if (!m_part) {
				m_part = PartList.Static.FindPart(m_partSlot.AssignedPartName, m_partSlot.AssignedPackageName,
					category);
			}
		...
		private Material GetMaterialForBaking(Material sourceMaterial) {
			...
			if (CharacterUtility.skinShaderNames.Contains(sourceMaterial.shader.name)) {
				...
				if (m_category == SlotCategory.BodySkin) {
					bakeMaterial.SetTexture(CharacterUtility.skinDetailsShaderId,
						sourceMaterial.GetTexture(CharacterUtility.skinDetailsShaderId));
					bakeMaterial.SetColor(CharacterUtility.detailsColorShaderId,
						sourceMaterial.GetColor(CharacterUtility.detailsColorShaderId));
				}
