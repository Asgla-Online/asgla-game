...
namespace CharacterCreator2D.Utilities.Humanoid {
	...
	public class HumanoidBaker : CharacterBaker {
		...
		[SerializeField] private SpritePacker m_spritePacker = new SpritePacker();
				...
				switch (category) {
					...
					default:
						...
						for (int i = 0; i < sprites.Count; i++) {
							...
							if (!m_packedSlotCategory.TryGetValue(category,
								out List<SpritePacker.TextureSprite> result)) {
			...
			foreach (var category in CharacterUtility.slotCategories) {
				...
				switch (category) {
					...
					case SlotCategory.MainHand: {
						...
						character.transform.AssignSpriteColor(SetupData.weaponFXLinks[category],
							character.slots[category].color1);
						break;
					}
		...
		private Texture GenerateTexture2D(RenderTexture renderTexture) {
			...
			Texture2D texture = new Texture2D(renderTexture.width, renderTexture.height, m_spritePacker.textureFormat,
				m_spritePacker.textureMipChain);
