...
namespace CharacterCreator2D.Utilities {
	...
	public class SpritePacker {
		...
		[Range(0.5f, 0.99999f)] public float scaleDownMultiplier = 0.99f;
		...
		private static bool GenerateAtlas(Vector2[] sizes,
			int padding,
			int atlasSize,
		...
		private static void ReadTexture(RenderTexture renderTexture,
			Texture sourceTexture,
			Rect sourcePixelRect,
			Rect targetPixelRect) {
			Rect sourceRect = new Rect(sourcePixelRect.x / sourceTexture.width,
				sourcePixelRect.y / sourceTexture.height, sourcePixelRect.width / sourceTexture.width,
				sourcePixelRect.height / sourceTexture.height);
			Rect targetRect = new Rect(targetPixelRect.x / renderTexture.width,
				targetPixelRect.y / renderTexture.height, targetPixelRect.width / renderTexture.width,
				targetPixelRect.height / renderTexture.height);
