...
namespace CharacterCreator2D.UI {
	public class UIExportPNG : MonoBehaviour {
		...
		[Header("UI References")] public Camera previewCam;
		...
		[Space] public InputField inputFPS;
		...
		[Header("Export Settings")] public int targetFPS = 12;
		...
		public void EraseConfig() {
			...
			if (File.Exists(path)) {
				runtimeDialog.DisplayDialog("Delete Preset",
					"Are you sure you want to remove '" + configName + "' preset?", true);
		...
		public void SaveConfig(string configName) {
			...
			if (File.Exists(path)) {
				runtimeDialog.DisplayDialog("Save Preset",
					"Preset '" + configName + "' already exist. Do you want to replace it?", true);
		...
		void UpdateSpriteSize() {
			renderTexture = new RenderTexture(width * superSampling, height * superSampling, 24,
		...
		private IEnumerator SaveSequence() {
			...
			for (int i = 0; i < frameCount; i++) {
...
#if UNITY_EDITOR
				UnityEditor.EditorUtility.DisplayProgressBar("Export Sequence",
					"Exporting frame " + i.ToString() + "/" + frameCount.ToString(), progress);
#else
				...
				Texture2D tex2D = new Texture2D(width * superSampling, height * superSampling, TextureFormat.RGBA32,
					false);
				RenderTexture.active = renderTexture;
				previewCam.Render();
				tex2D.ReadPixels(new Rect(0, 0, width * superSampling, height * superSampling), 0, 0, false);
