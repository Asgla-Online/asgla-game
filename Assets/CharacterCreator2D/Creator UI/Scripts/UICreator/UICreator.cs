...
namespace CharacterCreator2D.UI {
	public class UICreator : MonoBehaviour {
		...
		[HideInInspector] public CharacterData defaultCharacter;
#if UNITY_EDITOR
		...
		public void NewCharacter() {
			dialog.DisplayDialog("New Character",
				"Are you sure you want to create a new character? All unsaved changes will be lost.", true);
		...
		IEnumerator ie_savecharaasprefab() {
			...
			if (!string.IsNullOrEmpty(path)) {
				...
				dialog.DisplayDialog("Save Character",
					"Character is saved to <color=#178294>" + path + "</color> succesfully.");
			}
#endif
			_processing = false;
		}
		...
		public void LoadCharacterFromPrefab() {
			...
			if (!string.IsNullOrEmpty(path)) {
				...
				dialog.DisplayDialog("Load Character",
					"Character from <color=#178294>" + path + "</color> is loaded succesfully.");
			}
		...
		public void RandomizePart(SlotCategory slotCategory) {
			...
			character.EquipPart(slotCategory,
				_availparts[slotCategory][Random.Range(0, _availparts[slotCategory].Count)]);
		}
			...
			foreach (SlotCategory c in System.Enum.GetValues(typeof(SlotCategory))) {
				...
				switch (c) {
					...
					case SlotCategory.Skirt:
						...
						break;
					default:
						character.EquipPart(c,
							RollDice() < _RANDSETTINGS[c]
								? availparts[c][Random.Range(0, availparts[c].Count)]
								: nullpart);
						break;
				}
			}
		...
		public void RandomizeColor(params SlotCategory[] excludedCategory) {
			...
			if (!excludedcats.Contains(SlotCategory.Armor))
				character.SetPartColor(SlotCategory.Armor, colortheme[Random.Range(0, colortheme.Count)],
					colortheme[Random.Range(0, colortheme.Count)], colortheme[Random.Range(0, colortheme.Count)]);
			if (!excludedcats.Contains(SlotCategory.Pants))
				character.SetPartColor(SlotCategory.Pants, colortheme[Random.Range(0, colortheme.Count)],
					colortheme[Random.Range(0, colortheme.Count)], colortheme[Random.Range(0, colortheme.Count)]);
			if (!excludedcats.Contains(SlotCategory.Helmet))
				character.SetPartColor(SlotCategory.Helmet, colortheme[Random.Range(0, colortheme.Count)],
					colortheme[Random.Range(0, colortheme.Count)], colortheme[Random.Range(0, colortheme.Count)]);
			if (!excludedcats.Contains(SlotCategory.Gloves))
				character.SetPartColor(SlotCategory.Gloves, colortheme[Random.Range(0, colortheme.Count)],
					colortheme[Random.Range(0, colortheme.Count)], colortheme[Random.Range(0, colortheme.Count)]);
			if (!excludedcats.Contains(SlotCategory.Boots))
				character.SetPartColor(SlotCategory.Boots, colortheme[Random.Range(0, colortheme.Count)],
					colortheme[Random.Range(0, colortheme.Count)], colortheme[Random.Range(0, colortheme.Count)]);
			if (!excludedcats.Contains(SlotCategory.Cape))
				character.SetPartColor(SlotCategory.Cape, colortheme[Random.Range(0, colortheme.Count)],
					colortheme[Random.Range(0, colortheme.Count)], colortheme[Random.Range(0, colortheme.Count)]);
			if (!excludedcats.Contains(SlotCategory.MainHand))
				character.SetPartColor(SlotCategory.MainHand, colorpool[Random.Range(0, colorpool.Count)],
					colorpool[Random.Range(0, colorpool.Count)], colorpool[Random.Range(0, colorpool.Count)]);
			if (!excludedcats.Contains(SlotCategory.OffHand))
				character.SetPartColor(SlotCategory.OffHand, colorpool[Random.Range(0, colorpool.Count)],
					colorpool[Random.Range(0, colorpool.Count)], colorpool[Random.Range(0, colorpool.Count)]);
			if (!excludedcats.Contains(SlotCategory.Skirt))
				character.SetPartColor(SlotCategory.Skirt, colortheme[Random.Range(0, colortheme.Count)],
					colortheme[Random.Range(0, colortheme.Count)], colortheme[Random.Range(0, colortheme.Count)]);
			// Set face colors
			...
			if (!excludedcats.Contains(SlotCategory.Eyebrow)) {
				if (flashiness < 80)
					...
					character.SetPartColor(SlotCategory.Eyebrow, ColorCode.Color1,
						haircolor[Random.Range(0, haircolor.Count)]);
			}
			...
			if (!excludedcats.Contains(SlotCategory.Mouth)) {
				if (character.bodyType == BodyType.Female)
					character.SetPartColor(SlotCategory.Mouth, ColorCode.Color1,
						lipscolor[Random.Range(0, lipscolor.Count)]);
				else
