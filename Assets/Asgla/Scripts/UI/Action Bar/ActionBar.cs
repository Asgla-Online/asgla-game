...
namespace Asgla.UI.ActionBar {
    public class ActionBar : MonoBehaviour {
        ...
        public SkillMain GetSkillBySlotID(int slotID) => _slots.Where(s => s.ID == slotID).FirstOrDefault();
        ...
        public void OnSkillClick(SkillMain slot) {
            ...
            if (player.Target() == null) {
                if (player.Targets().Count == 0 && skillInfo.Target != SkillTarget.SELF) {
                    ...
                    Main.Singleton.Game.Chat.ReceiveChatMessage(1, 
                        string.Format("<size=22>{1}</size> <b><color=#{0}>{3}</color></b> <color=#{2}>{4}</color>", CommonColorBuffer.ColorToString(Color.red), DateTime.Now.ToShortTimeString(), CommonColorBuffer.ColorToString(Color.white), "game", "select target"));
                    return;
                }
            ...
            if (player.Target().Type() == Data.Entity.EntityType.PLAYER && player.Area().IsSafe() && skillInfo.Target != SkillTarget.SELF) {
                //TODO: Chat
                Main.Singleton.Game.Chat.ReceiveChatMessage(1,
                    string.Format("<size=22>{1}</size> <b><color=#{0}>{3}</color></b> <color=#{2}>{4}</color>", CommonColorBuffer.ColorToString(Color.green), DateTime.Now.ToShortTimeString(), CommonColorBuffer.ColorToString(Color.white), "game", "safe area, please no"));
                return;
            }
            ...
            if (Main.Singleton.Game.CastBar.IsCasting) {
                //TODO: Chat
                Main.Singleton.Game.Chat.ReceiveChatMessage(1,
                    string.Format("<size=22>{1}</size> <b><color=#{0}>{3}</color></b> <color=#{2}>{4}</color>", "ffa500", DateTime.Now.ToShortTimeString(), CommonColorBuffer.ColorToString(Color.white), "game", "casting.. wait"));
                return;
            }
            ...
            if (player.Target() != null) {
                if (!player.IsNear(player.Target(), skillInfo.Range)) {
                    //TODO: Chat
                    Main.Singleton.Game.Chat.ReceiveChatMessage(1,
                        string.Format("<size=22>{1}</size> <b><color=#{0}>{3}</color></b> <color=#{2}>{4}</color>", "ffa500", DateTime.Now.ToShortTimeString(), CommonColorBuffer.ColorToString(Color.white), "game", "target distance not in range"));
                    return;
                }
            } else {
            ...
            switch (skillInfo.CastType) {
                case SkillCastType.CAST:
                    Main.Singleton.Game.CastBar.StartCasting(skillInfo, skillInfo.CastTime, Time.time + skillInfo.CastTime, target, string.Join(",", targets));
                    break;
                default:
                    Main.Singleton.Request.Send("Combat", slot.ID, target, string.Join(",", targets));
                    break;
