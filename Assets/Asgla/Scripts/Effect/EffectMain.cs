...
namespace Asgla.Effect {
	public class EffectMain : MonoBehaviour {
		...
		public static EffectMain Singleton { get; private set; }

		public void AddAsset(EffectData e) {
			_effect.Add(e);
		}

		public EffectData GetByName(string name) {
			return _effect.Where(v => v.Name == name).First();
		}

		public void SetAsset(EffectMain data) {
			Singleton = data;
		}
		...
		public IEnumerator SpawmAsset(GameObject asset, AvatarMain from, AvatarMain target) {
			...
			Vector2 spawn = loop
				? new Vector2(from.Position().x, from.Position().y + 1.3f)
				: new Vector2(target.Position().x, target.Position().y + 1.3f);
