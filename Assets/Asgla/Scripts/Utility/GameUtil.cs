...
namespace Asgla.Utility {
	public class GameUtil {
		...
		public static List<AvatarMain> FindTarget(int quantity,
			double scale,
			double range,
			AvatarMain from,
			HashSet<AvatarMain> near,
			HashSet<AvatarMain> ignore) {
			return near.Where(a =>
					AvatarDistance(scale, a.Position(), from.Position()) <= range && a != from && !ignore.Contains(a))
